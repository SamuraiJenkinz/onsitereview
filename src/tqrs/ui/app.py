"""Main Streamlit application for TQRS."""
...
def render_welcome_view() -> None:
    ...
    st.title("📊 TQRS - Ticket Quality Review System")
...
def render_export_section() -> None:
    ...
    with col1:
        ...
        st.download_button(
            "⬇️ Download JSON",
            data=json_data,
            file_name=f"tqrs_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
    ...
    with col2:
        ...
        st.download_button(
            "⬇️ Download CSV",
            data=csv_data,
            file_name=f"tqrs_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
            mime="text/csv",
            use_container_width=True,
        )
    ...
    with col3:
        ...
        if state.summary:
            ...
            st.download_button(
                ...
                data=batch_html,
                file_name=f"tqrs_batch_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html",
                mime="text/html",
                use_container_width=True,
            )
